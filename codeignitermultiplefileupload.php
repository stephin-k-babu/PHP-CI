...
<?php
function file_upload ( $config, $files )
        {
            $this->load->library('upload');
            $this->upload->initialize($config);
            if ( $files = $this->upload->do_multi_upload($files, TRUE) )
            {
                return $fname = $this->upload->get_multi_upload_data();
            }
        }
?>
//sample config
